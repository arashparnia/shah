    ...
    for (int i = 0; i<10; i++) {
        for (int j = 0; j<12; j++) {
            for (int k = 0; k<20; k++) {
                ...
                MOVES[i][j][k].CAPTURED.POS=OFFBOARD;
            }
        }
        ...
        for (int l = 0; l < 150; l++) {
            ...
            SortedComputerMoveList[i][l].CAPTURED.POS=0;
            SortedComputerMoveList[i][l].CAPTURED.TYPE=0;
            SortedComputerMoveList[i][l].CAPTURED.GAS=0;
            SortedHumanMoveList[i][l].FROM=0;
            SortedHumanMoveList[i][l].TO=0;
            SortedHumanMoveList[i][l].EFFECT=0;
            SortedHumanMoveList[i][l].CAPTURED.POS=0;
            SortedHumanMoveList[i][l].CAPTURED.TYPE=0;
            SortedHumanMoveList[i][l].CAPTURED.GAS=0;
        }
    }
}
void resetMOVES(int depth,int side){
    ...
    for (int j = 0; j<12; j++) {
        for (int k = 0; k<20; k++) {
            ...
            MOVES[depth][j][k].CAPTURED.POS=OFFBOARD;
        }
    }
}
void setup(BOARD *b){
    ...
    b->PEICES[WhiteKnightLeft].POS= 34;     b->PEICES[WhiteKnightLeft].TYPE = WhiteKnightLeft;      b->PEICES[WhiteKnightLeft].GAS=3;   b->PEICES[WhiteKnightLeft].captured=false;
    b->PEICES[WhiteBishopLeft].POS=23;      b->PEICES[WhiteBishopLeft].TYPE = WhiteBishopLeft;      b->PEICES[WhiteBishopLeft].GAS=3;   b->PEICES[WhiteBishopLeft].captured=false;
    b->PEICES[WhiteQueen].POS = 24;         b->PEICES[WhiteQueen].TYPE = WhiteQueen;                b->PEICES[WhiteQueen].GAS=3;        b->PEICES[WhiteQueen].captured=false;
    b->PEICES[WhiteKing].POS = 25;          b->PEICES[WhiteKing].TYPE = WhiteKing;                  b->PEICES[WhiteKing].GAS=3;         b->PEICES[WhiteKing].captured=false;
    b->PEICES[WhiteBishopRight].POS = 26;   b->PEICES[WhiteBishopRight].TYPE = WhiteBishopRight;    b->PEICES[WhiteBishopRight].GAS=3;  b->PEICES[WhiteBishopRight].captured=false;
    b->PEICES[WhiteKnightRight].POS = 36;   b->PEICES[WhiteKnightRight].TYPE = WhiteKnightRight;    b->PEICES[WhiteKnightRight].GAS=3;  b->PEICES[WhiteKnightRight].captured=false;


    b->PEICES[BlackKnightLeft].POS=74;      b->PEICES[BlackKnightLeft].TYPE = BlackKnightLeft;      b->PEICES[BlackKnightLeft].GAS=3;   b->PEICES[BlackKnightLeft].captured=false;
    b->PEICES[BlackBishopLeft].POS=83;      b->PEICES[BlackBishopLeft].TYPE = BlackBishopLeft;      b->PEICES[BlackBishopLeft].GAS=3;   b->PEICES[BlackBishopLeft].captured=false;
    b->PEICES[BlackQueen].POS=84;           b->PEICES[BlackQueen].TYPE = BlackQueen;                b->PEICES[BlackQueen].GAS=3;        b->PEICES[BlackQueen].captured=false;
    b->PEICES[BlackKing].POS=85;            b->PEICES[BlackKing].TYPE = BlackKing;                  b->PEICES[BlackKing].GAS=3;         b->PEICES[BlackKing].captured=false;
    b->PEICES[BlackBishopRight].POS=86;     b->PEICES[BlackBishopRight].TYPE = BlackBishopRight;    b->PEICES[BlackBishopRight].GAS=3;  b->PEICES[BlackBishopRight].captured=false;
    b->PEICES[BlackKnightRight].POS=76;     b->PEICES[BlackKnightRight].TYPE = BlackKnightRight;    b->PEICES[BlackKnightRight].GAS=3;  b->PEICES[BlackKnightRight].captured=false;
}
    ...
    for(rank = RANK_1;rank <= RANK_7;++rank){
        ...
        for (file = FILE_A; file <= FILE_H;++file){
            ...
            if (b->POS[sq]!= EMPTY){
                ...
                printf("%1c ",piece2Char[b->PEICES[b->POS[sq]].TYPE]);
            } else {
                if (black) printf(" ## ");
                else printf(" -- ");
            }
...
void generateKnightMoves(BOARD *b,int p,int depth){
    if (b->PEICES[p].GAS>0 && !b->PEICES[p].captured){
        MOVE m;m.FROM=OFFBOARD;m.TO=OFFBOARD;m.EFFECT=0;m.CAPTURED.POS=OFFBOARD;m.CAPTURED.TYPE=0; int moveindex=0;
        m.FROM = b->PEICES[p].POS;
        bool side = SIDE(p);
        signed int KnightPsitions[8]={-21,-19,-12,-8,8,12,19,21};
        int t_pos;
        for (int i = 0; i<8; i++) {
            t_pos = b->PEICES[p].POS;
...
void generateQueenMoves(BOARD *b,int p,int depth){
    if (b->PEICES[p].GAS>0 && !b->PEICES[p].captured){
        MOVE m;m.FROM=OFFBOARD;
        m.TO=OFFBOARD;
        m.EFFECT=0;
        m.CAPTURED.POS=OFFBOARD;
        m.CAPTURED.TYPE=0;
        int moveindex=0;
        m.FROM = b->PEICES[p].POS;
        bool side = SIDE(p);
...
void generateHumanMoves(BOARD *b,int depth){
    if (!b->PEICES[BlackKnightLeft].captured)   generateKnightMoves(b, BlackKnightLeft,depth);
    if (!b->PEICES[BlackBishopLeft].captured)   generateBishopMoves(b, BlackBishopLeft,depth);
    if (!b->PEICES[BlackQueen].captured)        generateQueenMoves(b, BlackQueen,depth);
    if (!b->PEICES[BlackKing].captured)         generateKingMoves(b, BlackKing,depth);
    if (!b->PEICES[BlackBishopRight].captured)  generateBishopMoves(b, BlackBishopRight,depth);
    if (!b->PEICES[BlackKnightRight].captured)  generateKnightMoves(b, BlackKnightRight,depth);
    for (int i = 0; i < 150; i++) {
        SortedHumanMoveList[depth][i].FROM=0;
        SortedHumanMoveList[depth][i].TO=0;
        SortedHumanMoveList[depth][i].EFFECT=0;
        SortedHumanMoveList[depth][i].CAPTURED.POS=0;
        SortedHumanMoveList[depth][i].CAPTURED.TYPE=0;
        SortedHumanMoveList[depth][i].CAPTURED.GAS=0;
    }
    int index = 0;
    ...
    //            if ((m.TO != m.FROM) && b->PEICES[m.FROM].GAS >0 && (m.EFFECT == ENEMY_CAPTURE)){
    //                SortedHumanMoveList[depth][index] = m;
    //                index++;
    //            }
    //        }
    ...
    //        for (int i = 0; i < 20; i++) {
    //            MOVE m = MOVES[depth][j][i];
    //            if ((m.TO != m.FROM) && b->PEICES[m.FROM].GAS >0 && ((m.EFFECT == NOTHING)||(m.EFFECT == UNKNOWN))){
    //                SortedHumanMoveList[depth][index] = m;
    //                index++;
    //            }
    //        }
    ...
    //        for (int i = 0; i < 20; i++) {
    //            MOVE m = MOVES[depth][j][i];
    //            if ((m.TO != m.FROM) && b->PEICES[m.FROM].GAS >0 && (m.EFFECT  == OUT_OF_GAS)){
    //                SortedHumanMoveList[depth][index] = m;
    //                index++;
    //            }
    //        }
...
void generateComputerMoves(BOARD *b,int depth){
    ...
    if (!b->PEICES[WhiteKnightLeft].captured)   generateKnightMoves (b, WhiteKnightLeft,depth);
    if (!b->PEICES[WhiteBishopLeft].captured)   generateBishopMoves (b, WhiteBishopLeft,depth);
    if (!b->PEICES[WhiteQueen].captured)        generateQueenMoves  (b, WhiteQueen,depth);
    if (!b->PEICES[WhiteKing].captured)         generateKingMoves   (b, WhiteKing,depth);
    if (!b->PEICES[WhiteBishopRight].captured)  generateBishopMoves (b, WhiteBishopRight,depth);
    if (!b->PEICES[WhiteKnightRight].captured)  generateKnightMoves (b, WhiteKnightRight,depth);
    for (int i = 0; i < 150; i++) {
        SortedComputerMoveList[depth][i].FROM=0;
        SortedComputerMoveList[depth][i].TO=0;
        SortedComputerMoveList[depth][i].EFFECT=0;
        SortedComputerMoveList[depth][i].CAPTURED.POS=0;
        SortedComputerMoveList[depth][i].CAPTURED.TYPE=0;
        SortedComputerMoveList[depth][i].CAPTURED.GAS=0;
    }
    int index = 0;
    ...
    //        for (int i = 0; i < 20; i++) {
    //            MOVE m = MOVES[depth][j][i];
    //            if ((m.TO != m.FROM) && (m.EFFECT == ENEMY_CAPTURE) && b->PEICES[m.FROM].GAS >0){
    //                SortedComputerMoveList[depth][index] = m;
    //                index++;
    //            }
    //        }
...
void doMove(BOARD *b,MOVE *m){
    if (b->PEICES[b->POS[m->FROM]].GAS > 0){
        ...
        if ((m->FROM != m->TO) ) {
            if (b->POS[m->TO]!=EMPTY) {
                m->CAPTURED.TYPE = b->PEICES[b->POS[m->TO]].TYPE;
                m->CAPTURED.GAS = b->PEICES[b->POS[m->TO]].GAS;
                m->CAPTURED.POS = b->PEICES[b->POS[m->TO]].POS;
                b->PEICES[b->POS[m->TO]].captured=true;
                m->GASHISTORY = b->PEICES[b->POS[m->FROM]].GAS;
...
void undoMove(BOARD *b,MOVE *m){
    b->PEICES[b->POS[m->TO]].POS = m->FROM;
    b->PEICES[b->POS[m->TO]].GAS = m->GASHISTORY;
    b->POS[m->FROM] = b->POS[m->TO] ;
    if (m->CAPTURED.TYPE == EMPTY){
        b->POS[m->TO] = EMPTY;
    }else{
        b->PEICES[m->CAPTURED.TYPE].POS = m->CAPTURED.POS;
        b->PEICES[m->CAPTURED.TYPE].TYPE = m->CAPTURED.TYPE;
        b->PEICES[m->CAPTURED.TYPE].captured = false;
        b->POS[m->TO] = m->CAPTURED.TYPE;
    }
}
...
int inCheck(BOARD *b){
    ...
    for (int i = 0; i<150 ;i++){
        if (b->POS[SortedComputerMoveList[0][i].TO] == BlackKing) incheck += 5555;
        if (b->POS[SortedHumanMoveList[0][i].TO] == WhiteKing) incheck += -5555;
    }
    return incheck;
}
int EvaluateBoard(BOARD *b){
    ...
    for (int i =1;i<13;i++) {
        if(!b->PEICES[i].captured && b->PEICES[i].GAS>0) {
...
int IsGameOver(BOARD *b){
    ...
    if (b->PEICES[WhiteKing].captured) score = -99999;
    if (b->PEICES[WhiteKnightLeft].captured && b->PEICES[WhiteKnightRight].captured &&
        b->PEICES[WhiteBishopLeft].captured && b->PEICES[WhiteBishopRight].captured &&
        b->PEICES[WhiteQueen].captured      && b->PEICES[WhiteKing].GAS == 0) score = -99999;
    if (b->PEICES[WhiteKnightLeft].GAS<1    && b->PEICES[WhiteKnightRight].GAS<1 &&
        b->PEICES[WhiteBishopLeft].GAS<1    && b->PEICES[WhiteBishopRight].GAS<1 &&
        b->PEICES[WhiteQueen].GAS<1         && b->PEICES[WhiteKing].GAS<1) score = -99999;

    if (b->PEICES[BlackKing].captured) score = 99999;
    if (b->PEICES[BlackKnightLeft].captured && b->PEICES[BlackKnightRight].captured &&
        b->PEICES[BlackBishopLeft].captured && b->PEICES[BlackBishopRight].captured &&
        b->PEICES[BlackQueen].captured      && b->PEICES[BlackKing].GAS == 0) score = 99999;
    if (b->PEICES[BlackKnightLeft].GAS<1    && b->PEICES[BlackKnightRight].GAS<1  &&
        b->PEICES[BlackBishopLeft].GAS<1    && b->PEICES[BlackBishopRight].GAS<1  &&
        b->PEICES[BlackQueen].GAS<1         && b->PEICES[BlackKing].GAS<1 ) score = 99999;
...
int quiesce(BOARD *b,int depth,int alpha, int beta) {
    ...
    for (int i =0;i<150;i++){
        ...
        if (b->PEICES[b->POS[q_m.FROM]].GAS > 0 && q_m.EFFECT == ENEMY_CAPTURE ){
                doMove(b,&q_m);
...
int Max(BOARD *b,int depth,int alpha,int beta){
    if (IsGameOver(b) != 0) return EvaluateBoard(b);
    ...
    for (int i =0;i<150;i++){
        ...
        t_m.EFFECT = SortedComputerMoveList[depth][i].EFFECT;
            if (b->PEICES[b->POS[t_m.FROM]].GAS > 0) {
                ...
                undoMove(b, &t_m);
                //if (b->POS[SortedComputerMoveList[depth][i].TO] == BlackKing) t_score = 555555;
                if( t_score >= beta ) return beta;   // fail hard beta-cutoff
                else if( t_score > alpha ) alpha = t_score;
...
int Min(BOARD *b,int depth,int alpha,int beta){
    if (IsGameOver(b) != 0) return EvaluateBoard(b);
    if(depth > MAXDEPTH || t.GetTicks() >MAXTIME ) return quiesce(b,depth,alpha,beta);
    int t_score;
    ...
    for (int i =0;i<150;i++){
        ...
        t_m.EFFECT = SortedHumanMoveList[depth][i].EFFECT;
            if (b->PEICES[b->POS[t_m.FROM]].GAS > 0) {
                ...
                undoMove(b, &t_m);
                //if (b->POS[SortedHumanMoveList[depth][i].TO] == WhiteKing)t_score = -555555;
                if( t_score <= alpha )return alpha; // fail hard alpha-cutoff
                else if( t_score < beta )beta = t_score;
                //if (t_score < best.score) best.score = t_score;
            }
    }
    return beta;
...
//    }
    ...
    //generateBishopMoves(&board,BlackQueen,0);
    //for (int i = 0 ; i <18; i ++){
    //    MOVE m= MOVES[0][BlackQueen][i];
    //    printMove(m);
    //
    //}
